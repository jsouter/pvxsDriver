...
dbLoadDatabase("$(TOP)/dbd/pvxsDriverApp.dbd")
pvxsDriverApp_registerRecordDeviceDriver(pdbbase)
...
# Create a pvxsDriver
# pvxsDriverConfig(portName, pvName, maxBuffers, maxMemory, priority, stackSize)
pvxsDriverConfig("$(PORT)", "$(PVNAME)", 0, 0, 0, 0)
#asynSetTraceMask $(PORT) 0 0xFF
#asynSetTraceInfoMask $(PORT) 0 0x7
dbLoadRecords("$(pvxsDriver)/db/pvxsDriver.template","P=$(PREFIX),R=cam1:,PORT=$(PORT),ADDR=0,TIMEOUT=1")

# Create a standard arrays plugin, set it to get data from the pvxsDriver.
...
set_requestfile_path("$(pvxsDriver)/pvxsDriverApp/Db")
